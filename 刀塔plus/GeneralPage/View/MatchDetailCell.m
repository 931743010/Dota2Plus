...
-(void)cellWithDetailModel:(MatchDetailModel *)model
{
    ...
    [_heroIcon setImageWithURL:[NSURL URLWithString:model.heroIcon] placeholderImage:[UIImage imageNamed:@"heroPlaceHolder"]];
    ...
    if([model.playerIcon isEqualToString:@"http://cdn.dota2.com.cn/steamcommunity/public/images/avatars/fe/fef49e7fa7e1997310d705b2a6158ff8dc1cdfeb_full.jpg"])
        _playerIcon.image=[UIImage imageNamed:@"defalutPlayerIcon.jpg"];
    else
        [_playerIcon setImageWithURL:[NSURL URLWithString:model.playerIcon] placeholderImage:[UIImage imageNamed:@"defalutPlayerIcon.jpg"]];
...
//    给玩家图像添加手势
    ...
    _lastHitLabel.text=[NSString stringWithFormat:@"正反补:%@",model.lastHit];
    if(model.isMVP)
    {
        _mvpLabel.text=@"MVP";
