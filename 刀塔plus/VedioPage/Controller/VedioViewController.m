...
-(void)createClassBtn
{
    ...
    NSArray *classBtnTitles=@[@"剑雪封喉",@"Dota视频",@"Dota2视频"/*,@"直播"*/];
            ...
            if([result isKindOfClass:[NSDictionary class]])
            {
                ...
                if(array.count>0)
                {
                    ...
                    for(NSDictionary *dict in array)
                    {
                        ...
                        if(model)
                        {
                            if([model.name isEqualToString:@"剑雪封喉"]||[model.name isEqualToString:@" DOTA2视频"])
                                continue;
...
//        [GMDCircleLoader setOnView:self.view withTitle:@"Loading..." animated:YES];
        JXFHVideoModel *model=nil;
        if(_jxfhDataArray.count>indexPath.row)
        {
            model=_jxfhDataArray[indexPath.row];
            play.urlString=model.realUrl;
            play.videoTitle=model.title;
